...
public class AppTest extends Application {
	...
	public void start(Stage primaryStage) {
		try {
			...
			HitBox hb1 = new HitBox((short) 50, (short) 50,
					new MotionPoint(800, 200, new Vector2D(), new Vector2D(1, 0), new RotationParameters()),
					new RotationParameters(0, -2, 0));
			HitBox hb2 = new HitBox((short) 50, (short) 50,
					new MotionPoint(600, 200, new Vector2D(), new Vector2D(0, 1), new RotationParameters()),
					new RotationParameters());
			HitBox hb3 = new HitBox((short) 50, (short) 50,
					new MotionPoint(1000, 200, new Vector2D(), new Vector2D(), new RotationParameters()),
					new RotationParameters(0, 5, 0));
			HitBox hb4 = new HitBox((short) 50, (short) 50,
					new MotionPoint(500, 500, new Vector2D(), new Vector2D(), new RotationParameters(0, 0.0001f, 0)),
					new RotationParameters());
			hbList.add(hb1);
			hbList.add(hb2);
			hbList.add(hb3);
			...
			rec3.rotateProperty()
					.bind(hb3.getSelfRotationParameters().angleProperty().multiply(360).divide(Math.PI).divide(2));
			rec1.rotateProperty()
					.bind(hb1.getSelfRotationParameters().angleProperty().multiply(360).divide(Math.PI).divide(2));
			...
			rec4.rotateProperty()
					.bind(hb4.getSelfRotationParameters().angleProperty().multiply(360).divide(2 * Math.PI));
			...
			protected Task createTask() {
				return new Task() {
					...
					protected Void call() throws Exception {
						while (true) {
							if (MathUtilitaires.round(hb.getCenterPoint().getVelocity()
									* Math.cos(hb.getCenterPoint().direction())) == -10) {
			...
			protected Task createTask() {
				return new Task() {
					...
					protected Void call() throws Exception {
						while (true) {
							if (MathUtilitaires.round(hb.getCenterPoint().getVelocity()
									* Math.sin(hb.getCenterPoint().direction())) == -10) {
