...
public class HitBox implements Comparable<HitBox>, AddressString {
    ...
    private Line2D[] getLinesOfPerimeter() {
        ...
        Point topRight = MathUtilitaires.rotatePoint(new Point(x, y), centerPoint, angleOfRotation);
        Point topLeft = MathUtilitaires.rotatePoint(new Point(-x, y), centerPoint, angleOfRotation);
        Point botLeft = MathUtilitaires.rotatePoint(new Point(-x, -y), centerPoint, angleOfRotation);
        Point botRight = MathUtilitaires.rotatePoint(new Point(x, -y), centerPoint, angleOfRotation);

        Line2D l1 = new Line2D.Double(centerPoint.getX() + topRight.getX(), centerPoint.getX() + topRight.getY(), centerPoint.getX() + botRight.getX(), centerPoint.getY() + botRight.getY());
        Line2D l2 = new Line2D.Double(centerPoint.getX() + botRight.getX(), centerPoint.getX() + botRight.getY(), centerPoint.getX() + botLeft.getX(), centerPoint.getY() + botLeft.getY());
        Line2D l3 = new Line2D.Double(centerPoint.getX() + botLeft.getX(), centerPoint.getX() + botLeft.getY(), centerPoint.getX() + topLeft.getX(), centerPoint.getY() + topLeft.getY());
        Line2D l4 = new Line2D.Double(centerPoint.getX() + topLeft.getX(), centerPoint.getX() + topLeft.getY(), centerPoint.getX() + topRight.getX(), centerPoint.getY() + topRight.getY());

        Line2D[] linesOfPerimeter = {l1, l2, l3, l4};

        return linesOfPerimeter;
    }

    public boolean checkCollision(HitBox box) {
        boolean intersects = false;

        if (this.hitsOthers() == true || box.hitsOthers() == true) {
            ...
            for (int i = 0; i < l; i++) {
                for (int j = 0; j < k; j++) {
                    if (thislines[i].intersectsLine(boxlines[j])) {
                        intersects = true;
                        break;
                    }
                }
    ...
    public boolean equals(Object o) {
        ...
        if (width != hitBox.width) return false;
        return height == hitBox.height;
    }
    ...
    @Override
    public int hashCode() {
        int result = (31 * width) + (23 * height) + (17 * width * height);
        return result;
    }
    ...
    private void thread() {
        Thread t = new Thread(() -> {
            while (true) {
                //centerPoint.rotate(origin);
                centerPoint.move();
                System.out.println("vel : " + centerPoint.getVelocity());
                System.out.println(centerPoint.getX());
                System.out.println(centerPoint.getY());
                System.out.println();
                //System.out.println(centerPoint.x() + ", " + centerPoint.y() + "  Velocité : " + centerPoint.getVelocity());
                try {
                    Thread.sleep(500);
    ...
    public static void main(String[] args) throws ConstructorException {
        HitBox hb = new HitBox((short) 30, (short) 30, new MotionPoint(30, 30, new Vector2D(50, 0), new Vector2D(-5, 0), new RotationParameters((float) Math.PI, 0, 0)), new RotationParameters());
        hb.setOrigin(new MotionPoint(0, 0));
        hb.thread();
    }
